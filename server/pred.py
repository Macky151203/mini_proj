...
def predictresult(input_data):
    ...
    y_pred_dt = DTclassifier.predict(data2)
    ...
    y_pred_rf = RFclassifier.predict(data2)
    # yy = RFclassifier.predict(x_test2)
    # print("rf-", yy)
    ...
    sample_input2 = sc.transform(data2)
    NBclassifier = GaussianNB()
    NBclassifier.fit(x_train2, y_train2)
    y_pred_nb = NBclassifier.predict(sample_input2)
    result.append(y_pred_nb[0])
    print("Naive Bayes-", y_pred_nb)
